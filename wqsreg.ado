     ...
     forvalues s = `start'/`end' {	   
                                  ...
                                  forvalues j = 1/`n_mixt'{
								                        ...
								                        forvalues i = 1/`rh_rep' {
														 ...
														 display ""
                                                         display  "-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"
								                         display "Repeated Holdout Validation - Weights: "								 		
														 sum  Weights*
								 ...
								 graph combine box_Weights.gph coef_WQS_index_var.gph, title("rh - `rh_rep' repetitions")
...
program define wqsreg_no_rh , rclass
 ...
 mixture(varlist)  boot(integer) 
  ...
  if `boot'==1 { 
                ...
                WQS_single_boot Y_wqs Q_wqs_* `cvar',   b1_neg(`b1_neg')  n_conf(`n_conf') conv_maxiter(`conv_maxiter') conv_vtol(`conv_vtol') technique(`technique') model_fam(`model_fam')
