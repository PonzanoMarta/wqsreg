...
{phang}
{it:yvar} is the name of the variable representing the continuous outcome. 
...
{synopt :{opt saveWQSindex(integer)}}specify whether to save a dataset containing the WQS index for each observation: use 1 to save; the default is 0 (do not save){p_end}
...
{pstd}
{cmd:wqsreg} – enables users to fit WQS regression for continuous outcomes while allowing for the several flexible components of this framework
(e.g., confounders adjustment, direction specification, bootstrapping, and training-validation data splitting).
...
{pstd}Marta Ponzano [1], Stefano Renzetti [2], Andrea Bellavia [3]{p_end}

{pstd}[1] {it:Department of Health Sciences, University of Genoa, Genoa, Italy}{p_end}
{pstd}[2] {it:Department of Medicine and Surgery, University of Parma, Parma, Italy}{p_end}
{pstd}[3] {it:Department of Environmental Health, Harvard T.H. Chan School of Public Health}{p_end}
